...
for (const item of products) {
    if (params.name === item.name) {
        detailHtml = `
            ...
            <h1>${item.name}</h1>
            <div class="status">
                <p class="cate">Thể loại: <a href="">${item.category.charAt(0).toUpperCase() + item.category.slice(1)}</a></p>
                <p class="line">|</p>
            ...
            <form action="" class="order">
                ...
                <button class="button-effect" type="submit">Thêm vào giỏ hàng <i class='bx bx-right-arrow-alt'></i></button>
            </form>
            </div>
        `;

        titleHtml = `${item.category.toUpperCase()}`
    }
}
